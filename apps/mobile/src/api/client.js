...
export function normalizeBaseUrl(value) {
  ...
  if (!normalized) throw new ApiError('Enter the AkshaConnect server URL', 0, 'SERVER_URL_REQUIRED');
...
async function request(baseUrl, path, { token = '', method = 'GET', body, signal } = {}) {
  ...
  try {
    ...
    throw new ApiError(error?.message || 'Could not reach the AkshaConnect server', 0, 'NETWORK_ERROR');
  }
...
export function loginLocal(baseUrl, { workspaceCode, loginName, password }) {
  return request(baseUrl, '/api/v1/auth/local/login', {
    method: 'POST',
    body: { workspace_code: workspaceCode, login_name: loginName, password },
  });
}

export function logout(baseUrl, token) {
  return request(baseUrl, '/api/v1/auth/logout', { token, method: 'POST' });
}
